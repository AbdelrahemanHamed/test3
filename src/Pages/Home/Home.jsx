...
<Navbar/>
<a href="https://wa.me/41766025572"><img src={WhatsappIcon} className="WhatsappIcon" alt="" /></a>
<section className="hero-main-section">
  <div className="text">
    <h1><p className='Tilte-home'>Wangen </p>
      Pizza, Kebab und Kurier
    </h1>
    <h3 className="desc">Unser Geheimrezept: Qualität, Frische und Vielfalt!</h3>
    <button className="home-btn">Jetzt bestellen</button>
  </div>
  <div className="toggle-btn">
    <img src={!darkMode?light_mode:Dark_mood} alt="" className="dark-image-mode" onClick={()=>{toggleDarkMode()}} />
  </div>
   <div className="slide-bg">
    <span onClick={()=>{setSlid(backgroundImage)}}><span onClick={()=>changeColor()} style={{background:red && "#FF8C42"}}></span> </span>
    <span onClick={()=>{setSlid(backgroundImage2)}}><span onClick={()=>changeColor1()} style={{background:red1 && "#FF8C42"}}></span></span>
    <span onClick={()=>{setSlid(backgroundImage3)}}><span onClick={()=>changeColor2()} style={{background:red2 && "#FF8C42"}}></span></span>
   </div>
